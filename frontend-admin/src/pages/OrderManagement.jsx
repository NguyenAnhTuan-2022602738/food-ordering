...
export default function OrderManagement() {
  ...
  const getStatusConfig = (status) => {
    const configs = {
      ...
      DELIVERING: { color: 'text-indigo-600 bg-indigo-50 border-indigo-200', icon: Truck, label: 'Delivering' },
      COMPLETED: { color: 'text-green-600 bg-green-50 border-green-200', icon: Package, label: 'Completed' },
      CANCELLED: { color: 'text-red-600 bg-red-50 border-red-200', icon: X, label: 'Cancelled' }
    }
                  ...
                  return (
                    <tr key={order.id} className="hover:bg-gray-50/50 transition-colors group">
                      ...
                      <td className="px-6 py-4">
                        ...
                        <span className="text-xs text-gray-500 block">{order.items?.length} items</span>
                      </td>
                      <td className="px-6 py-4">
            ...
            <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
              <div>
                ...
                <p className="text-gray-500 text-sm flex items-center gap-1">
                  <Clock size={14} /> Placed on {new Date(selectedOrder.createdAt).toLocaleString('vi-VN')}
                </p>
              </div>
              <button 
            ...
            <div className="p-6 overflow-y-auto flex-1 custom-scrollbar">
              <div className="grid md:grid-cols-2 gap-8 mb-8">
                ...
                <div className="space-y-4">
                  <h3 className="font-bold text-gray-900 flex items-center gap-2">
                    <User size={18} className="text-indigo-600" /> Customer Details
                  </h3>
                ...
                <div className="space-y-4">
                  <h3 className="font-bold text-gray-900 flex items-center gap-2">
                    <Package size={18} className="text-indigo-600" /> Order Summary
                  </h3>
              ...
              <div>
                <h3 className="font-bold text-gray-900 mb-4 flex items-center gap-2">
                  <CheckCircle size={18} className="text-indigo-600" /> Update Status
                </h3>
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
                  {[
                    { status: 'CONFIRMED', label: 'Confirm', color: 'bg-blue-600 hover:bg-blue-700', allowedFrom: ['PENDING'] },
                    { status: 'PREPARING', label: 'Prepare', color: 'bg-purple-600 hover:bg-purple-700', allowedFrom: ['CONFIRMED'] },
                    { status: 'DELIVERING', label: 'Ship', color: 'bg-indigo-600 hover:bg-indigo-700', allowedFrom: ['PREPARING'] },
                    { status: 'COMPLETED', label: 'Complete', color: 'bg-green-600 hover:bg-green-700', allowedFrom: ['DELIVERING'] },
                    { status: 'CANCELLED', label: 'Cancel', color: 'bg-red-600 hover:bg-red-700', allowedFrom: ['PENDING', 'CONFIRMED', 'PREPARING'] }
                  ].map((action) => {
